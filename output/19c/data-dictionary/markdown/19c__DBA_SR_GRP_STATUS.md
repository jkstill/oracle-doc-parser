...
Previous Next JavaScript must be enabled to correctly display this content Refresh operations are controlled using the DBMS_SYNC_REFRESH package. Related View USER_SR_GRP_STATUS provides information on the current refresh operations for the current synchronous refresh groups in the database which are owned by the current user. Its columns are the same as those in DBA_SR_GRP_STATUS . See Also: " USER_SR_GRP_STATUS " Oracle Database PL/SQL Packages and Types Reference for more information about the DBMS_SYNC_REFRESH package