...
Previous Next JavaScript must be enabled to correctly display this content It does this by showing the complete sequence of redo log streams processed or created on the local system, across all role transitions. (After a role transition, a new log stream is started and the log stream sequence number is incremented by the new primary database.). This view is for logical standby databases only. Note: In a CDB, this view shows data only when queried in the root. Note: In a CDB, this view shows data only when queried in the root.