...
Previous Next JavaScript must be enabled to correctly display this content Related Views DBA_ERRORS describes the current errors on all stored objects in the database. USER_ERRORS describes the current errors on the stored objects owned by the current user. This view does not display the OWNER column. See Also: " DBA_ERRORS " " USER_ERRORS " See Also: " DBA_ERRORS " " USER_ERRORS "