...
Previous Next JavaScript must be enabled to correctly display this content Related Views DBA_VIEWS_AE describes all views (across all editions) in the database. USER_VIEWS_AE describes the views (across all editions) owned by the current user. This view does not display the OWNER column. See Also: " DBA_VIEWS_AE " " USER_VIEWS_AE " See Also: " DBA_VIEWS_AE " " USER_VIEWS_AE "