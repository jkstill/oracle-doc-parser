...
Previous Next JavaScript must be enabled to correctly display this content There is one row per queue per sender. The values are reset to zero when the database (or instance in an Oracle RAC environment) restarts.