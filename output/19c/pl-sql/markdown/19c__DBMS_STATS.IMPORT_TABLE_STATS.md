---
id: 19c__DBMS_STATS.IMPORT_TABLE_STATS
name: DBMS_STATS.IMPORT_TABLE_STATS
...
| partname | VARCHAR2 | IN | Name of the table partition. If the table is partitioned and if partname is NULL , then global and partition table statistics are imported. |
| stattab | VARCHAR2 | IN | Identifies the user statistics table that describes where to retrieve the statistics. |
| statid | VARCHAR2 | IN | Specifies the ID associated with these statistics within stattab . |
| cascade | BOOLEAN | IN | Indicates whether to import column and index statistics for this table. The default is TRUE . |
