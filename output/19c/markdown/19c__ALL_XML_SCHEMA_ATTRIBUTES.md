...
Related Views DBA_XML_SCHEMA_ATTRIBUTES describes all the attributes and their properties accessible to the current user in the database. USER_XML_SCHEMA_ATTRIBUTES describes all the attributes and their properties owned by the current user. This view does not display the OWNER column. See Also: " DBA_XML_SCHEMA_ATTRIBUTES " " USER_XML_SCHEMA_ATTRIBUTES " Oracle XML DB Developer’s Guide for information about registering an XML schema with Oracle XML DB Oracle XML DB Developer’s Guide for information about restrictions for an XML schema URL See Also: " DBA_XML_SCHEMA_ATTRIBUTES " " USER_XML_SCHEMA_ATTRIBUTES " Oracle XML DB Developer’s Guide for information about registering an XML schema with Oracle XML DB Oracle XML DB Developer’s Guide for information about restrictions for an XML schema URL